...
var DaySlot = _react2['default'].createClass({
  ...
  _selectable: function _selectable() {
    ...
    var maybeSelect = function maybeSelect(box) {
      ...
      if (onSelecting) {
        if (_utilsDates2['default'].eq(current.startDate, start, 'minutes') && _utilsDates2['default'].eq(current.endDate, end, 'minutes') || onSelecting({ start: start, end: end }) === false) return;
      }
